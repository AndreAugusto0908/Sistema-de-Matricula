...
import { AlunoForm } from "../../forms/AlunoForm"; // ajuste o import do form de aluno

export function SearchAddAluno({ setFiltro }: { setFiltro: (value: string) => void }) {
      ...
      {isModalOpen && (
        <div className="fixed inset-0 w-full h-full flex items-center justify-center bg-black/50 z-50">
          <div className="max-w-md w-full bg-white rounded-md p-6">
            <AlunoForm closeModal={() => setIsModalOpen(false)} />
          </div>
        </div>
      )}
    </>
